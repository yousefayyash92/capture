...
@interface PSCamera () <AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
...
@property (nonatomic, copy) void (^didRecordCompletionBlock)(PSCamera *camera, NSURL *outputFileUrl, NSError *error);
@end

NSString *const LLSimpleCameraErrorDomain = @"LLSimpleCameraErrorDomain";

@implementation PSCamera
...
- (void)attachToViewController:(UIViewController *)vc holderView:(UIView *)holderView withFrame:(CGRect)frame {
    [vc addChildViewController:self];
    self.view.frame = frame;
    [holderView addSubview:self.view];
    [self didMoveToParentViewController:vc];
}

- (void)start
{
    [PSCamera requestCameraPermission:^(BOOL granted) {
        if(granted) {
            // request microphone permission if video is enabled
            if(self.videoEnabled) {
                [PSCamera requestMicrophonePermission:^(BOOL granted) {
                    if(granted) {
                        [self initialize];
                    }
                    else {
...
- (void)initialize
{
    if(!_session) {
        ...
        videoDataoutput.videoSettings = [ NSDictionary dictionaryWithObject: framePixelFormat
...
-(void)capture:(void (^)(PSCamera *camera, UIImage *image, NSDictionary *metadata, NSError *error))onCapture exactSeenImage:(BOOL)exactSeenImage animationBlock:(void (^)(AVCaptureVideoPreviewLayer *))animationBlock
{
    if(!self.session) {
        NSError *error = [NSError errorWithDomain:LLSimpleCameraErrorDomain
...
-(void)capture:(void (^)(PSCamera *camera, UIImage *image, NSDictionary *metadata, NSError *error))onCapture exactSeenImage:(BOOL)exactSeenImage {
...
-(void)capture:(void (^)(PSCamera *camera, UIImage *image, NSDictionary *metadata, NSError *error))onCapture
{
...
- (void)startRecordingWithOutputUrl:(NSURL *)url didRecord:(void (^)(PSCamera *camera, NSURL *outputFileUrl, NSError *error))completionBlock
{
